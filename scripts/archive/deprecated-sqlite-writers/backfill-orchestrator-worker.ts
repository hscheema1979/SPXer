...
import { expectedContractsForProfile } from '../../src/instruments/backfill-routing';
import {
  buildMtfForSymbol,
  listSymbolsForDate,
...
async function main(): Promise<void> {
  ...
  try {
    gaps = findMissingDates(db, profile.underlyingSymbol, {
      ...
      anchorSymbol: 'SPX',
      options: {
        prefix: profile.optionPrefix,
        expectedContractsPerDate: expectedContractsForProfile(profile),
      },
    });
  } catch (e: unknown) {
  ...
  // Dates with missing underlying OR missing option chain go through the
  // worker (Phase A) — the worker is idempotent and will only re-fetch what's
  // needed. Pure MTF/indicator gaps stay in-process (Phase B).
  const rawMissing = pending.filter(g => g.missingRaw || g.missingOptions === true);
  const mtfMissing = pending.filter(g => !g.missingRaw && g.missingOptions !== true);
