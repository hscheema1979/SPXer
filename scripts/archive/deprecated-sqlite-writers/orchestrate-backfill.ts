...
import { hasParquetDate, symbolToProfileId } from '../../src/storage/parquet-reader';
...
/**
 * Generate a US-equity trading calendar (weekdays minus hardcoded holidays)
 * inclusive on both ends. Used when --start and --end are provided so the
 * orchestrator can find gaps for new profiles even when no anchor symbol
 * has data in replay_bars yet.
 */
...
function generateTradingCalendar(start: string, end: string): string[] {
  const days: string[] = [];
...
async function main(): Promise<void> {
  ...
  try {
    ...
    // Trading-day universe: when start+end are both provided, generate a real
    // trading calendar (weekdays minus US holidays) so we don't depend on SPX
    // having full coverage in replay_bars (most SPX history lives in parquet,
    // not SQLite). When start/end are absent, fall back to anchoring on SPX
    // coverage in replay_bars (legacy behavior).
    const explicitCalendar = (args.start && args.end)
      ? generateTradingCalendar(args.start, args.end)
      : undefined;
    const gaps = findMissingDates(replayDb, profile.underlyingSymbol, {
      start: args.start,
      end: args.end,
      anchorSymbol: 'SPX',
      tradingDates: explicitCalendar,
    });

    // Drop dates that already have parquet coverage for this profile — bar
    // data may live in parquet rather than replay_bars (the replay engine
    // reads parquet first), so missing-from-SQLite ≠ missing-overall. This
    // prevents pointless re-fetches for SPX/NDX which have ~270 parquet days
    // but only ~5 SQLite days from daily-backfill copies.
    const parquetProfileId = symbolToProfileId(profile.underlyingSymbol);
    const beforeParquet = gaps.length;
    const filteredGaps = gaps.filter(g => !hasParquetDate(parquetProfileId, g.date));
    const skippedByParquet = beforeParquet - filteredGaps.length;

    const pending = filteredGaps.filter(hasWorkPending);
    ...
    console.log(`  parquet covered: ${skippedByParquet}`);
    console.log(`  fully covered : ${(gaps.length - skippedByParquet) - pending.length}`);
