...
// Blocker-1 fix: the 4 fast-path cases stay byte-identical (SPX must, incl.
// .brc + protected). Anything else → resolve from sweep-registry.json by
// parsing profileId as `{symbol}-{dte}dte`. A registry-resolved profile points
// underlyingParquetDir at its OWN dir; when that parquet is absent
// getUnderlyingDay() fetches the underlying straight from Polygon (Blocker-2
// fix), so a brand-new ticker is fully self-contained.
function resolveTarget(profileId: string | undefined): BackfillTarget {
  switch (profileId) {
    case 'ndx-0dte':
      return { profileId: 'ndx-0dte', prefix: 'NDXP', underlyingDbSymbol: 'NDX', underlyingPolygonTicker: 'I:NDX', strikeInterval: 10, bandHalfWidthDollars: 500, dte: 0, underlyingParquetDir: 'ndx' };
    case 'spy-1dte':
      return { profileId: 'spy-1dte', prefix: 'SPY', underlyingDbSymbol: 'SPY', underlyingPolygonTicker: 'SPY', strikeInterval: 1, bandHalfWidthDollars: 10, dte: 1, underlyingParquetDir: 'spy' };
    case 'qqq-1dte':
      return { profileId: 'qqq-1dte', prefix: 'QQQ', underlyingDbSymbol: 'QQQ', underlyingPolygonTicker: 'QQQ', strikeInterval: 1, bandHalfWidthDollars: 10, dte: 1, underlyingParquetDir: 'qqq' };
    case 'spx-0dte':
    case undefined:
      return { profileId: 'spx-0dte', prefix: 'SPXW', underlyingDbSymbol: 'SPX', underlyingPolygonTicker: 'I:SPX', strikeInterval: 5, bandHalfWidthDollars: 100, dte: 0, underlyingParquetDir: 'spx' };
    default:
      return resolveFromRegistry(profileId);
  }
}

function resolveFromRegistry(profileId: string): BackfillTarget {
  const m = /^([a-z]+)-(\d+)dte$/.exec(profileId);
  if (!m) throw new Error(`Bad profile id '${profileId}' — expected '{symbol}-{dte}dte'.`);
  const symbol = m[1].toUpperCase(), dte = parseInt(m[2], 10);
  let reg: any;
  try { reg = JSON.parse(fs.readFileSync(path.resolve(__dirname, '../diag/sweep-registry.json'), 'utf8')); }
  catch (e: any) { throw new Error(`Cannot read sweep-registry.json: ${e.message}`); }
  const p = (reg.profiles || []).find((x: any) => String(x.symbol).toUpperCase() === symbol && Number(x.dte) === dte)
         || (reg.profiles || []).find((x: any) => String(x.symbol).toUpperCase() === symbol);
  if (!p) throw new Error(`No sweep-registry.json entry for '${profileId}'. Add { symbol:"${symbol}", dte:${dte}, class, strikeInterval, optionPrefix, underlyingPolygonTicker, bandHalfWidthDollars }.`);
  const cls = p.class === 'index' ? 'index' : 'etf';
  return {
    profileId,
    prefix: p.optionPrefix || symbol,
    underlyingDbSymbol: symbol,
    underlyingPolygonTicker: p.underlyingPolygonTicker || (cls === 'index' ? `I:${symbol}` : symbol),
    strikeInterval: Number(p.strikeInterval) || 1,
    bandHalfWidthDollars: Number(p.bandHalfWidthDollars) || (cls === 'index' ? 100 : 10),
    dte,
    underlyingParquetDir: profileId, // own dir; cache-miss → fetch from Polygon (sole vendor)
  };
}
...
/**
 * Fetch 1m option bars from Polygon — the SOLE vendor (ThetaData removed
 * 2026-05-17, no vendor fallback exists). `src` is always 'polygon'.
 */
export async function fetchOptionBars(dbSymbol: string, date: string): Promise<{ bars: PolygonBar[]; src: 'polygon' }> {
  const pbars = await fetchPolygonOptionBars(dbSymbol, date);
  return { bars: pbars, src: 'polygon' };
}
/** @deprecated legacy ThetaData-era name — Polygon is primary/sole. Use fetchOptionBars. */
export const fetchOptionBarsWithFallback = fetchOptionBars;
 ...
 */
// Blocker-2 fix: fetch the underlying 1m straight from Polygon (RTH-filtered,
// 429-aware) when no 0DTE parquet exists — ported from eod-backfill.ts so a
// brand-new ticker is self-contained. Polygon is the sole vendor.
...
function bandFromClose(date: string, close: number, target: BackfillTarget): UnderlyingDay['range'] {
  const baseStrike = Math.round(close / target.strikeInterval) * target.strikeInterval;
  return { date, close, minStrike: baseStrike - target.bandHalfWidthDollars, maxStrike: baseStrike + target.bandHalfWidthDollars };
}
...
async function getUnderlyingDay(date: string, target: BackfillTarget): Promise<UnderlyingDay | null> {
  ...
  if (!fs.existsSync(fp)) {
    // No pre-existing 0DTE parquet (genuinely new ticker) → Polygon underlying.
    const rows = await fetchPolygonUnderlying(target.underlyingPolygonTicker, target.underlyingDbSymbol, date);
    if (!rows.length) return null;
    const close = Number(rows[rows.length - 1].close);
    return { range: bandFromClose(date, close, target), rows };
  }
...
async function backfillDate(
  ...
  for (const side of sides) {
    for (const strike of strikes) {
      ...
      try {
        const { bars, src } = await fetchOptionBars(dbSymbol, date);
