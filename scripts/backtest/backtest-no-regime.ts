...
const PARAMS = {
  ...
  // RSI thresholds
  rsiOversoldTrigger: 20,        // escalate when RSI < this
  rsiOverboughtTrigger: 80,      // escalate when RSI > this
  rsiEmergencyOversold: 15,      // override regime gate
  rsiEmergencyOverbought: 85,    // override regime gate
  rsiMorningEmergencyOversold: 10,  // stricter for morning
  rsiMorningEmergencyOverbought: 92,
  ...
  // Position management
  stopPct: 0.50,                 // 50% of premium
  tpMultiplier: 10,              // 10x entry
  maxRiskPerTrade: 300,
...
function isSignalAllowed(regime: Regime, rsi: number, direction: 'call' | 'put', minute: number): boolean {
  ...
  if (minute >= PARAMS.noTradeMinute) return false;
  return true;
  // Emergency override
  ...
  switch (regime) {
    ...
    case 'GAMMA_EXPIRY':
      return false;                          // only emergency overrides in gamma zone
  }
}
...
interface DayResult {
  date: string;
  trades: Trade[];
  signals: Array<{ ts: number; et: string; rsi: number; regime: Regime; direction: string; allowed: boolean; strike?: string; reason?: string }>;
  totalPnl: number;
  winRate: number;
...
function backtestDay(db: Database.Database, date: string): DayResult | null {
  ...
  for (let i = 0; i < bars.length; i++) {
    ...
    if (!selected) {
      signalEntry.reason = 'No OTM contract in $0.50-$3.00 band';
      signals.push(signalEntry);
      lastSignalBar = i;
      continue;
    }

    if (isEmergencyOversold || isEmergencyOverbought) emergenciesCaught++;
...
function generateReplayMd(result: DayResult): string {
  ...
  md += `## Signals\n\n`;
  md += `| Time | RSI | Regime | Direction | Allowed | Strike/Reason |\n`;
  md += `|------|-----|--------|-----------|---------|---------------|\n`;
  for (const s of signals) {
    const mark = s.allowed ? '✅' : '🚫';
    md += `| ${s.et} | ${s.rsi.toFixed(1)} | ${s.regime} | ${s.direction} | ${mark} | ${s.strike || s.reason || ''} |\n`;
  }
