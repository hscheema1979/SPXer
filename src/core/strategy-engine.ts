...
// ── New Signal-Only Types ────────────────────────────────────────────────────

/** HMA state for signal detection — no positions, no trade state. */
...
/**
 * SPX bars for direction gate + exit monitoring.
 * NOTE: detectSignal() on SPX bars is used for EXIT tracking only.
 * ENTRY signals come from detectSignals() in signal-detector.ts (option contract bars).
 */
...
/**
 * SPX HMA cross state — used by live agents for:
 *   - directionState: requireUnderlyingHmaCross gate on entry
 *   - exitState: scannerReverse exit trigger
 *
 * NOT the primary entry signal. Entry is driven by detectSignals() on option bars.
 */
export interface SignalResult {
  ...
  exitState: {
    ...
    prevSlow: number | null;
    lastBarTs: number | null;
    freshCross: boolean;  // true only on the cycle a new closed bar produced the cross
  };
}

// ── HMA Cross Detection ────────────────────────────────────────────────────

export interface HmaCrossResult {
  cross: Direction | null;
  prevFast: number | null;
  prevSlow: number | null;
  lastBarTs: number | null;
 ...
 */
export function detectHmaCross(
  bars: CoreBar[],
  fastPeriod: number,
  slowPeriod: number,
  prevFast: number | null,
...
/**
 * detectSignal() — SPX underlying HMA cross detector.
 * Used by live agents for TWO purposes only:
 *   1. Exit monitoring: exitState.freshCross triggers scannerReverse exits
 *   2. Direction gate: directionState.cross feeds requireUnderlyingHmaCross
 *
 * NOT the entry signal source. Entry signals come from detectSignals() in
 * signal-detector.ts, which runs HMA crosses on option contract bars.
 * Both live agents and the replay system use detectSignals() for entries.
 */
export function detectSignal(
  ...
  return {
    directionState: {
      cross: dirResult.cross,
      prevFast: dirResult.prevFast,
      prevSlow: dirResult.prevSlow,
      lastBarTs: dirResult.lastBarTs,
      freshCross: dirResult.freshCross,
    },
    exitState: {
      cross: exitResult.cross,
      prevFast: exitResult.prevFast,
      prevSlow: exitResult.prevSlow,
      lastBarTs: exitResult.lastBarTs,
      freshCross: exitResult.freshCross,
    },
  };
}
...
export function tick(
  ...
  for (const [posId, pos] of state.positions) {
    ...
    const exitCtx: ExitContext = {
      ...
      hmaCrossFresh: exitResult.freshCross,
 ...
 */
export function getFlipDirection(
  reason: ExitReason,
