/**
 * Model Clients — Uses Anthropic SDK for Claude models, direct HTTP for third-party.
 *
 * Claude models use @anthropic-ai/sdk (picks up ANTHROPIC_API_KEY or OAuth automatically).
 * Third-party models (Kimi, GLM, MiniMax) use direct HTTP with their API keys.
 *
 * Scanners (Tier 1):
 *   - Kimi K2.5   → direct HTTP (api.kimi.com)
 *   - GLM-5       → direct HTTP (api.z.ai)
 *   - MiniMax M2.7 → direct HTTP (api.minimax.io)
 *   - Claude Haiku → Anthropic SDK
 *
 * Judge (Tier 2):
 *   - Claude Haiku/Sonnet/Opus → Anthropic SDK
 */
...
function getAnthropicClient(): Anthropic {
  if (!_anthropic) {
    _anthropic = new Anthropic();  // Picks up ANTHROPIC_API_KEY automatically
  }
  return _anthropic;
}
...
export async function askModel(config: ModelConfig, systemPrompt: string, userPrompt: string, timeoutMs = 60000): Promise<string> {
  ...
  try {
    ...
    const result = await askModelViaAnthropicSdk(config, systemPrompt, userPrompt, controller.signal);
    clearTimeout(timeoutId);
    return result;
  } catch (err) {
    clearTimeout(timeoutId);
...
// ---------------------------------------------------------------------------
// Anthropic SDK implementation for Claude models
// ---------------------------------------------------------------------------

async function askModelViaAnthropicSdk(config: ModelConfig, systemPrompt: string, userPrompt: string, signal?: AbortSignal): Promise<string> {
  if (signal?.aborted) {
