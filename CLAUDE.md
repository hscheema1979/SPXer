...
- `npm run test` (`vitest run`) — **781 passing / 0 failing** across 65 files
...
npm run test             # vitest run   (781 passing)
...
Tests mirror `src/` and `scripts/` under `tests/`. Vitest with `globals: true`, `node` environment, 10s timeout. **781 tests pass.**
...
- `npm run test` must stay green before commit (currently 781/781)
