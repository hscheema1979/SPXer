...
- `npm run test` (`vitest run`) — **850 passing / 0 failing** across 68 files
...
npm run test             # vitest run   (850 passing)
...
Tests mirror `src/` and `scripts/` under `tests/`. Vitest with `globals: true`, `node` environment, 10s timeout. **850 tests pass.**
...
- `npm run test` must stay green before commit (currently 850/850)
