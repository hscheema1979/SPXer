...
- `npm run test` (`vitest run`) — **783 passing / 0 failing** across 65 files
...
npm run test             # vitest run   (783 passing)
...
Tests mirror `src/` and `scripts/` under `tests/`. Vitest with `globals: true`, `node` environment, 10s timeout. **783 tests pass.**
...
### Where to Start
1. **Read `CODE-REVIEW-2026-08-09.md`** for the current accurate status (the 2026-05-22 review is superseded), and **`docs/DATA-STORES.md`** for what writes which data store (one writer per store).
...
- `npm run test` must stay green before commit (currently 783/783)
