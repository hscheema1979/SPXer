...
- `npm run test` (`vitest run`) — **778 passing / 0 failing** across 64 files
...
npm run test             # vitest run   (778 passing)
...
Tests mirror `src/` and `scripts/` under `tests/`. Vitest with `globals: true`, `node` environment, 10s timeout. **778 tests pass.**
...
- `npm run test` must stay green before commit (currently 778/778)
